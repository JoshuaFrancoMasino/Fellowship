...
interface PinFormModalProps {
  ...
  initialPin?: Pin | null; // For editing existing pins
  initialLat?: number; // For creating new pins
  initialLng?: number; // For creating new pins
}

const PinFormModal: React.FC<PinFormModalProps> = ({
  isOpen,
  onClose,
  onSubmit,
  initialPin,
  ...
  const [selectedTribe, setSelectedTribe] = useState<TribeName>('Dan');
  ...
  const isEditMode = !!initialPin;
  const lat = initialPin?.lat ?? initialLat;
  const lng = initialPin?.lng ?? initialLng;

  useEffect(() => {
    if (isOpen) {
      if (isEditMode && initialPin) {
        // Initialize form with pin data for editing
        setDescription(initialPin.description);
        setImages(initialPin.images || []);
        setUploadedPaths(initialPin.storage_paths || []);
        
        // Find the tribe color that matches the pin color
        const tribeEntry = Object.entries(TRIBE_COLORS).find(([_, color]) => color === initialPin.pin_color);
        setSelectedTribe(tribeEntry ? tribeEntry[0] as TribeName : 'Dan');
        
        // Set location data
        setLocationData({
          continent: initialPin.continent,
          country: initialPin.country,
          state: initialPin.state,
          locality: initialPin.locality,
        });
      } else {
        // Reset form for new pin creation
        resetForm();
      }
        ...
        <div className="p-6">
          ...
          <div className="flex space-x-3">
            ...
            <button
              ...
              className="flex-1 px-4 py-3 bg-gradient-to-r from-yellow-400 to-orange-400 text-white rounded-lg hover:from-yellow-500 hover:to-orange-500 disabled:opacity-50 disabled:cursor-not-allowed transition-all font-medium flex items-center justify-center space-x-2"
            >
